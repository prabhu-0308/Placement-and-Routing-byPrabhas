...
def new_displacement(prev_modules):
    if len(prev_modules) == 0 or 1:
        return []

    prev_modules = sorted(prev_modules, key=lambda i: i['y'])
    ...
    for obj in prev_modules:
        if flag == 0:
            flag = 1
            ini_obj = obj
            vec.append({"vec_x": 0, "vec_y": 0})
        else:
            vec_x = obj["x"] - ini_obj["x"]
            vec_y = obj["y"] - ini_obj["y"]
            vec.append({"vec_x": vec_x, "vec_y": vec_y})
    return vec